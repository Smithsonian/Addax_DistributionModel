...
In this excercise, we will follow general steps detailed in:
...
We will use [R](https://cran.r-project.org/) for all analyses to create a species distribution model, highlighting the variables important in predicting addax occurrence. Like other exercises in [R](https://cran.r-project.org/), we will first load the necessary packages to perform the analyses. Use `help()` for any operations that you don't understand or that require additional information.
...
### Making the Plot Look a Little Prettier

Can we improve the look of these plots and plot the x-axis values with the original values?  The important item to remember here is that we need to transform the original values, which is easy enough since we have saved the parameter transformation values.
...
```{r Scale2, eval=T}
# Scale values. To transform, you need to manually do some raster math based on the values we saved previously
# Rememeber that centering and scaling meant that we:
# x - mean(x) / sd(x))

# Grab the mean and standard deviation values from the object saved above
Scale.Val
