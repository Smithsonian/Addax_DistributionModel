...
We can also take this one step further and plot our predictive surface on an interactive map, so that we have a better idea of its context in the real world and also provide a way to post our results on relevant social media outlets.
...
leaflet() %>%
  addTiles() %>%
  #addCircles(wild.lat,lng = wild.lat@coords[,1], lat = wild.lat@coords[,2], radius=1, color = "black", fillOpacity = 0.5, fill=TRUE, opacity = 0.5) %>%
