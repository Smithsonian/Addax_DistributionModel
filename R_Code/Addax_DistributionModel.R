...
# Calculate Presence threshold (Will be used later)
presenceAddax = round(sum(Sub.All2$obsAddax)/nrow(Sub.All2),digits=2)
...
# Like the Resource Selection exercise, we could use the dredge command to identify the best model.
...
# Graph the NDVI response
  # First, game the Min and Max values from the summary.data above
...
## ----SpatialCorr, eval=T-------------------------------------------------
# Graph the residuals to qualitative assess correlation
dat1 <- data.frame(Sub.Scale$X,Sub.Scale$Y,resids=resid(glm.Addax))
db <- dat1
# Make spatial
coordinates(dat1) <- c('Sub.Scale.X','Sub.Scale.Y')
# Plot as a bubble plot...qualitative assessment
bubble(dat1,zcol='resids',main="Spatial Representation of Residuals
- Addax -")
#xy <- data.frame(Sub.Scale$X,Sub.Scale$Y)

# Plot the semi-variance
var.mod1 <- variogram(resids~1,data=dat1,alpha=c(0,45,90,135))
plot(var.mod1)

## ----Val, eval=T---------------------------------------------------------
# Create a null model to compare with our full model
# Is this significantly different than the null (it better be)?
Null.Addax <- glm(obsAddax ~ 1, data = Sub.Scale, family = binomial(link="logit"))

anova(glm.Addax,test='Chisq')
anova(Null.Addax,glm.Addax,test='Chisq')
...
## ----Predict, eval=T-----------------------------------------------------
# Load NDVI data from flight survey
# Note that this file is 250-meter resolution (MOD13Q1 data product)
# The SRTM data is 30-m
# We need this data sources to have the same resolution in order to make the prediction. Otherwise, we will get an error.
...
# Assess the areas above the presence threshold (Calculated above)
presenceAddax
Addax.Thresh <- Addax.predict >= presenceAddax
plot(Addax.Thresh)

# Create a polygon for areas that are greater than the presence threshold
# This will be slow....
#pol <- rasterToPolygons(Addax.predict,fun=function(x){x >= presenceAddax},dissolve=TRUE)
