...
## ----Scale-------------------------------------------------------------------------------------------------------------------------------------------
# Scale the continuous variables that we'll include in the modeling
Occ <- Occ %>% mutate(
...
# Let's also scale our raster layers by the mean and standard deviation at each location so we don't forget to later
...
## ----Validation--------------------------------------------------------------------------------------------------------------------------------------
# Check for signs of over-disperson. 
# The value is close to 1: No evidence of over-dispersion.
glm.Addax$deviance/glm.Addax$df.residual

# Evaluate deviance residuals
# No strong evidence of lack of fit.  Most residuals values are around a value of 0.
...
# Remember that our image is projected to UTM 32 N WGS84
(proj.info <- crs(Addax.predict))

# Reproject raster to Lat/Long to overlay on interactive map
proj.LongLat <- CRS("+proj=longlat +datum=WGS84")
Addax.predict.lat <- projectRaster(Addax.predict, crs=proj.LongLat) # Use default resolution
